...
\paragraph{Требования в шаблонам html:} Динамический шаблон "Меню отчетов"
\begin{itemize}
    \item Шаблон содержит форму типа select для ввода типа отчета:
        \begin{itemize}
            \item По суммарной прибыли
            \item По количеству проданных билетов
            \item По средней цене билета за сеанс
        \end{itemize}
...
\paragraph{Гарантия:} пользователь получает возможность выбрать сеанс и купить билеты на места на сеанс.
...
\begin{enumerate}[start=1,label=\arabic*A.]
    \item Пользователь не авторизован
\end{enumerate}
\begin{enumerate}[start=1,label=\arabic*B.]
    \item Пользователь не имеет прав доступа
\end{enumerate}
\begin{enumerate}[start=3,label=\arabic*A.]
    \item На сеанс нет свободных мест
\end{enumerate}
